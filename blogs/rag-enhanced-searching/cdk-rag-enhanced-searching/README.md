...
대화이력을 저장하기 위해 DynamoDB의 테이블을 생성합니다.
...
Client 동작에 필요한 Query API를 생성합니다. 
...
Client에서 대화이력을 가져오기 위한 API를 생성합니다.
...
Client에서 대화이력 삭제를 위한 API를 생성합니다.
...
Web Socket을 위한 Enpoint인 API Gateway를 생성합니다.
...
구글의 credential을 저장하기 위하여 Secret manager를 생성합니다. 
...
S3에 업로드된 파일을 삭제시 OpenSearch의 index도 정리하도록 이벤트를 읽고 처리하는 lambda를 정의합니다.
...
Web Socket URL을 Client에서 조회할때에 사용하는 API를 정의합니다. 
